...
namespace STARK
{
    public class Gramaticas : Variables
    {
        ...
        public string SinonimoCmd(string _speech) //to lower == abrir bandeja
        {
            ...
            try
            {
                con = new OleDbConnection("Data Source = |DataDirectory|\\DataBase\\Data.accdb; Version = 3"); //conection stablished
                con.Open();

                cmd = new OleDbCommand("SELECT * FROM CmdInternos WHERE Sinonimo LIKE '%" + _speech + "%'", con); //abrir bandeja => no lo encuentra
                ...
                cmd = new OleDbCommand("SELECT * FROM CmdInternos WHERE Comando LIKE '%" + _speech + "%'", con); //abrir bandeja => si encuentra
                readerS = cmd.ExecuteReader();
        ...
        public string EjecutarComandosBD(string _speech)
        {
            ...
            con = new OleDbConnection("Data Source = |DataDirectory|\\DataBase\\Data.accdb; Version = 3");
            con.Open();

            //CmdSociales
            cmd = new OleDbCommand("SELECT * FROM CmdSociales WHERE Comando LIKE '%" + _speech + "%'", con);
            ...
            //CmdCarpetas
            cmd = new OleDbCommand("SELECT * FROM CmdCarpetas WHERE Comando LIKE '%" + _speech + "%'", con);
            readerS = cmd.ExecuteReader(CommandBehavior.SingleResult);
            if (readerS.Read())
            {
                confirmarcmd = readerS["Comando"].ToString().Split(new char[] { '+' });
            ...
            //Aplicaciones
            cmd = new OleDbCommand("SELECT * FROM CmdAplicaciones WHERE Comando LIKE '%" + _speech + "%'", con);
            readerS = cmd.ExecuteReader(CommandBehavior.SingleResult);
            if (readerS.Read())
            {
                confirmarcmd = readerS["Comando"].ToString().Split(new char[] { '+' });
            ...
            //Paginas Webs
            cmd = new OleDbCommand("SELECT * FROM CmdPaginasWebs WHERE Comando LIKE '%" + _speech + "%'", con);
            readerS = cmd.ExecuteReader(CommandBehavior.SingleResult);
            if (readerS.Read())
            {
                confirmarcmd = readerS["Comando"].ToString().Split(new char[] { '+' });
            ...
            //Arduino
            cmd = new OleDbCommand("SELECT * FROM CmdPuertoSerial WHERE Comando LIKE '%" + _speech + "%'", con);
            readerS = cmd.ExecuteReader(CommandBehavior.SingleResult);
            if (readerS.Read())
            {
                confirmarcmd = readerS["Comando"].ToString().Split(new char[] { '+' });
        ...
        public string EjecutarComandoBDInternos(string speech)
        {
            ...
            cmd = new OleDbCommand("SELECT * FROM CmdInternos WHERE Comando LIKE '%" + speech + "%'", con);
            readerS = cmd.ExecuteReader();
            if (readerS.Read())
            {
        ...
        {
            //buscar la historia de españa en google
